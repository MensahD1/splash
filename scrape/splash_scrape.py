...
def fetchGameTable(id,soup):
    ...
    "BLK":0,"TOV":0,"PF":0,"PTS":0,"opponentRoster":None,"wasPlayoff":False}
    ...
    for row in rows:
        ...
        details["2P"] =  int(column_data[9].get_text()) - int(details["3P"])
        details["2PA"] = int(column_data[10].get_text()) - int(details["3P"])
...
def fetchSeasons(soup):
    ...
    for season in season_rows:
        ...
        fetchGames(details["link"])
def fetchGames(soup):

    page = requests.get(soup)
    soup = BeautifulSoup(page.text, 'html.parser')

    fetchGameTable("all_pgl_basic",soup)
...
def loadLetter(letter):
    ...
    print("On Letter:",letter[-2].upper())
    ...
    for player in players:
        ...
        #try:
        fetchSeasons(result[1])
        print("Sucess",player.find_all('th')[0].get_text())
        #except (TypeError,IndexError):
            #ename = player.find_all('th')[0].get_text()
            #print("fetchSeasons:: Formatting error for player -",ename)
            #writeError(ename,2)
